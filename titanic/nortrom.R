...
#########____EXTRACTING USEFUL INFORMATION____###########

comb$Surname <- sapply(as.character(comb$Name), FUN=function(x) {strsplit(x, split='[,.]')[[1]][1]})
comb$title <- sapply(as.character(comb$Name), FUN=function(x) {strsplit(x, split='[,.]')[[1]][2]})
# comb$Survived <- as.numeric(comb$Survived)
...
comb$SLogL[comb$Age>=67 & comb$Age<74] <- comb$SLogL[comb$Age>=67 & comb$Age<74]-1.6
...
# linear_model <- lm(Survived~ SLogL , data = comb %>% filter(key=="train"))
# 
# linear_predict <- predict(linear_model,newdata=baratrain)
# linear_predict <- linear_predict - min(linear_predict)
# linear_predict <- linear_predict/max(linear_predict)
# thold <-quantile(linear_predict,0.6161616)      #0.6416
# filter <- as.numeric(linear_predict>thold)
# x <- comb$Survived[comb$key=="train"]
# 
# test_l_predict <- predict(linear_model, newdata = baratest)
# filter <- as.numeric(test_l_predict>thold)
# data <- data.frame(trap[,1],filter)
# colnames(data) <- c('PassengerId','Survived')
# write.csv(data,file='l_predict4.csv',row.names=FALSE)
# 
# rf_model <- randomForest(factor(Survived) ~ SLogL, data = baratrain,ntree=200)
# # md <- rf_model
# pred <- predict(rf_model,newdata=baratest)
# data <- data.frame(trap[,1],pred)
# colnames(data) <- c('PassengerId','Survived')
# write.csv(data,file='rf_predict4.csv',row.names=FALSE)
# 
# bstSparse <- xgboost(data = data.matrix(baratrain), label = baratrain$Survived, max.depth = 2, eta = 1, nthread = 2, nround = 2, objective = "binary:logistic")
# pred<- predict(bstSparse,newdata = data.matrix(baratest))
# pred[pred<0.5] <- 0
# pred[pred>0.5] <- 1
# data <- data.frame(trap[,1],pred)
# colnames(data) <- c('PassengerId','Survived')
# write.csv(data,file='xgb_predict1.csv',row.names=FALSE)
# sum(x==pred)/891
# 
# bstSparse <- xgboost(data = data.matrix(baratrain),booster="gblinear", label = baratrain$Survived, max.depth = 2, eta = 1, nthread = 2, nround = 2, objective = "binary:logistic")
# pred<- predict(bstSparse,newdata = data.matrix(baratest))
# pred[pred<0.5] <- 0
# pred[pred>0.5] <- 1
# data <- data.frame(trap[,1],pred)
# colnames(data) <- c('PassengerId','Survived')
# write.csv(data,file='xgb_predict2.csv',row.names=FALSE)
# sum(x==pred)/891
